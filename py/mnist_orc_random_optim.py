...
c = orc.Сlassifier(output_count, hidden_count)

# precalculation of all at once (requires a lot of memory)
hidden_train = (np.einsum('ik,jk->ij', x_train, kernels)).clip(0) / 784
hidden_test = (np.einsum('ik,jk->ij', x_test, kernels)).clip(0) / 784
